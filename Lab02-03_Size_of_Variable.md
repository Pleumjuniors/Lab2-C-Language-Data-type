...
int บน ESP32 ใช้ขนาด 4 ไบต์ (32 บิต)

1.2 จากผลลัพธ์ นักศึกษาสังเกตเห็นอะไรเมื่อค่าเกินขอบเขตของ int บน ESP32? 

เมื่อค่าที่กำหนดเกินขอบเขตของ int จะเกิด overflow หรือ underflow ทำให้ค่ากลับไปเริ่มนับจากขอบเขตตรงกันข้าม เช่น เกินค่าสูงสุดจะกลายเป็นค่าต่ำสุดและแสดงผลผิดปกติ.
...
- ความแตกต่างคือ double มีความแม่นยำสูงกว่า float เพราะเก็บทศนิยมได้ละเอียดกว่า

ควรใช้ double เมื่อต้องการคำนวณที่ต้องการความแม่นยำสูง เช่น งานวิทยาศาสตร์, การเงิน หรือเมื่อผลลัพธ์ที่ผิดพลาดจากการปัดทศนิยมมีผลกระทบมาก.
...
- char ใช้ขนาด 1 ไบต์ (8 บิต)

ค่าตัวเลข ASCII คือรหัสตัวเลขที่แทนอักขระ เช่น 'A' = 65, 'a' = 97 ทำให้คอมพิวเตอร์เก็บและประมวลผลตัวอักษรเป็นตัวเลขได้โดยตรง.
...
true จะแสดงผลเป็น 1 และ false จะแสดงผลเป็น 0 บน Serial Monitor.
...
- ขนาดชนิดข้อมูลจำนวนเต็มบน ESP32:
...
- บน ESP32, long มีขอบเขตเท่ากับ int คือ 32 บิต

หากต้องการเก็บจำนวนเต็มบวกที่ใหญ่ที่สุด ควรใช้ unsigned long long (64 บิต) เพื่อเก็บค่าบวกได้มากสุด.
...
byte ใช้ขนาด 1 ไบต์ (8 บิต)

ถ้ากำหนด myByte = 256; ผลลัพธ์จะเป็น 0 เพราะค่า 256 เกินขอบเขตของ byte (0–255) จึงเกิดการล้น (overflow) ทำให้ค่ากลับมาเริ่มนับที่ 0 ใหม่.
...
เพราะเมื่อหารกับ float หรือ double ระบบจะทำการคำนวณแบบทศนิยม จึงได้ผลลัพธ์มีจุดทศนิยม แต่เมื่อตัวหารถูกแปลงเป็น int การหารจะทำแบบจำนวนเต็ม ทำให้ทศนิยมถูกตัดทิ้งเหลือแต่จำนวนเต็ม.
...
ใช้ Type Casting เมื่อต้องการแปลงค่าข้อมูลจากชนิดหนึ่งเป็นอีกชนิดหนึ่งเพื่อให้การคำนวณหรือการเก็บข้อมูลถูกต้อง เช่น แปลง float เป็น int เมื่อต้องการตัดทศนิยม หรือแปลง int เป็น float เพื่อให้ได้ผลลัพธ์แบบทศนิยม.
